...
const MyPostedJobs = () => {
  ...
  const [jobs,setJobs] = useState([]);
    ...
    const fetchAllJobsData = async () =>{
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/myJobs/${user?.email}`)
      setJobs(data)
    }
    ...
    const handleDelete = async(id) =>{
      try{
        ...
        toast.success("Job post delete successfully..!")
        fetchAllJobsData()
        // if(data?.acknowledged){
        //   const filterData = [...jobs].filter(job => id !== job._id);
        //   setJobs(filterData)
        // }
        console.log(data)
      } catch (error) {
        ...
        (t) => (
          <div className='flex items-center gap-2'>
            ...
            <div className='flex gap-2'>
              ...
              onClick={() =>{
                toast.dismiss(t.id)
                handleDelete(id)}}>Yes</button>
              <button className='bg-green-400 p-2 rounded-md text-white' onClick={() => toast.dismiss(t.id)}>Cancel</button>
            </div>
          </div>
        )
      );
