...
const UpdateJob = () => {
  ...
  const {user} = useContext(AuthContext);
    const [job,setJobs] = useState({});
      ...
      useEffect(()=>{
        if(job?.date){
          const parseDate = new Date(job.date);
      ...
      const navigate = useNavigate();
        const handleSubmitForm = async e =>{
          ...
          // send the formData server side
          try{
            // make post request
             await axios.put(`${import.meta.env.VITE_API_URL}/job-update/${id}`, formData);
            // form reset
            form.reset();
            // navigate my posted job page
